...
interface GetADByVar {
  (target: HTMLElement, variable: string, offset?: number): number
}
 ...
 * @param {Number} offset 可选。将结果与此参数做"乘法"，以便移动小数点获得不同单位
 * @return {Number} 返回 `number` 类型的获取结果，可以是小数
 */
export const getADByVar: GetADByVar = (target, variable, offset) => computed(() => {
  const d = window.getComputedStyle(target).getPropertyValue(variable)
  const r = window.parseFloat(d)
  return offset ? r * offset : r
}).value
